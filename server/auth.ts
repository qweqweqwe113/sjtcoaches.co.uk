...
import { getAdminCredentials, updateAdminEmail, updateAdminPasswordHash } from "./db";
...
export const checkSessionFn = createServerFn({ method: "GET" }).handler(
  async (ctx: any) => {
    ...
    return { authed: sessions.has(sessionId) };
  },
);
...
export const getCredentialsFn = createServerFn({ method: "GET" }).handler(
  async () => {
    const mongoUri = process.env.MONGODB_URI;
    if (!mongoUri) throw new Error("MongoDB URI not configured");
